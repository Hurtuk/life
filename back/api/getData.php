		...
		if ($love) {
			...
			$ranges = $db->select("
				SELECT lv.id, CONCAT(p.firstname, ' ', p.lastname) AS title,
					...
					CONCAT('https://".$_SERVER['HTTP_HOST']."/lifeBO/images/loves/', lv.id, '.jpg') AS icon, p.birthday
			...
			$ranges = $db->select("
				SELECT a.id, a.title, a.comment, a.startDate, a.endDate, s.title AS structure,
						a.role, a.type, CONCAT(t.name, '|https://".$_SERVER['HTTP_HOST']."/lifeBO/images/tags/', t.icon, '.png') AS tag, s.color,
						CONCAT('https://".$_SERVER['HTTP_HOST']."/lifeBO/images/structures/', s.id, '.jpg') AS icon
				FROM activities a
				LEFT JOIN tags t
				ON t.id = a.idTag
